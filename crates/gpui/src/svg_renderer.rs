...
mod wasm {
    use crate::{AssetSource, DevicePixels, SharedString, Size};
    ...
    #[derive(Clone)]
    /// Stub SVG renderer for WASM (no-op).
    pub struct SvgRenderer {
    ...
    impl SvgRenderer {
        /// Creates a new SVG renderer stub.
        pub fn new(asset_source: Arc<dyn AssetSource>) -> Self {
            Self {
        ...
        pub(crate) fn render_alpha_mask(
            ...
            _bytes: Option<&[u8]>,
        ) -> anyhow::Result<Option<(Size<DevicePixels>, Vec<u8>)>> {
            Ok(None)
        }
    }
}
