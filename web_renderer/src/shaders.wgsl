// Zed Web Renderer Shaders
//
// Ported from crates/gpui/src/platform/wgpu/shaders.wgsl
//
// Key differences from the native GPUI shaders:
//   - No `enable dual_source_blending` (not broadly available in browsers)
//   - Subpixel sprites fall back to grayscale alpha blending
//   - Surface pipeline omitted (macOS-only CVPixelBuffer)
...
fn fs_path_rasterization(input: PathRasterizationVarying) -> @location(0) vec4<f32> {
    ...
    if (length(vec2<f32>(dx.x, dy.x)) < 0.001) {
        ...
        let grad = 2.0 * input.st_position.xx * vec2<f32>(dx.x, dy.x) - vec2<f32>(dx.y, dy.y);
        let f = input.st_position.x * input.st_position.x - input.st_position.y;
        let distance = f / length(grad);
...
// ===================================================================
// PATH SPRITE PIPELINE (composite from intermediate texture)
// ===================================================================
...
// ===================================================================
// SUBPIXEL SPRITE PIPELINE (grayscale fallback)
//
// Browser WebGPU does not broadly support dual_source_blending, which
// is needed for true per-channel subpixel alpha. We fall back to
// grayscale antialiasing: average the RGB coverage into a single
// alpha value and blend the tint color with that. The visual
// difference is subtle (slightly less crisp text on LCD displays)
// and acceptable for a streaming renderer.
// ===================================================================
...
@vertex
fn vs_subpixel_sprite(@builtin(vertex_index) vertex_id: u32, @builtin(instance_index) instance_id: u32) -> SubpixelSpriteVarying {
    let unit_vertex = vec2<f32>(f32(vertex_id & 1u), 0.5 * f32(vertex_id & 2u));
    let sprite = b_subpixel_sprites[instance_id];

    var out = SubpixelSpriteVarying();
    out.position = to_device_position_transformed(unit_vertex, sprite.bounds, sprite.transformation);
    out.tile_position = to_tile_position(unit_vertex, sprite.tile);
    out.color = hsla_to_rgba(sprite.color);
    out.clip_distances = distance_from_clip_rect_transformed(unit_vertex, sprite.bounds, sprite.content_mask, sprite.transformation);
...
@fragment
fn fs_subpixel_sprite(input: SubpixelSpriteVarying) -> @location(0) vec4<f32> {
    ...
    // Average the per-channel coverage into a single grayscale alpha.
    let alpha = dot(alpha_corrected, vec3<f32>(1.0 / 3.0));
    return blend_color(input.color, alpha);
}
